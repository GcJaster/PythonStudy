...
class TicTacToe:
    ...
    def __update_win_status(self) -> None:
    ...
    def show(self) -> None:
        ...
        # # передали координаты i, j, игрок - приняли -> обработали -> вернули статус игры
        # if (row > 0 and row < self._size - 1) and (col > 0 and col < self._size - 1):
        #     if all(x.value == self.__dict__[player] for x in self.pole[row]) or \
        #             all(x.value == player for x in (x[col] for x in self.pole)):
        #             self.win = 1 if player == human else self.win = 2
        #             return
        #
        # #кортеж свободных ячеек в которой будем рандомить ячейку для робота
